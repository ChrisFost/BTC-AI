...
class BucketManager:
    ...
    def handle_bucket_change(self, window, values):
        """
        Handle bucket selection changes in the UI by selecting the appropriate tab.
...
def handle_bucket_change(window, values):
    ...
    try:
        ...
        logger.info(f"Handling bucket change to: {bucket}")
        
        # --- Select the correct Tab in the TabGroup ---
        tab_map = {
            "Scalping": 0,
            "Short": 1,
            "Medium": 2,
        ...
        target_tab_index = tab_map.get(bucket, 0) # Default to Scalping index if not found
        
        if "-GOALS-TABGROUP-" in window.AllKeysDict:
            try:
                # Access the underlying Tkinter widget to select the tab by index
                window["-GOALS-TABGROUP-"].Widget.select(target_tab_index)
                logger.debug(f"Selected tab index {target_tab_index} for bucket {bucket}")
            except Exception as tk_error:
                logger.error(f"Error selecting tab for {bucket} using index {target_tab_index}: {tk_error}")
        else:
            logger.error("Goals TabGroup key '-GOALS-TABGROUP-' not found.")
        # --- End Tab Selection ---

        # Update look-back hint
        look_back_hints = {
        ...
        # update_bucket_goals might still be relevant for descriptions if keys are shared across tabs
